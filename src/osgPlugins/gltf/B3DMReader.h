...
*/
#ifndef B3DM_READER_H
#define B3DM_READER_H
    ...
    osg::Node* read(const std::string& location, const std::string& inputStream, const osgDB::Options* readOptions) const
    {
        ...
        if (magic != "b3dm")
        {
            ...
            if (compressor.valid())
            {
                ...
                {
                    OSG_WARN << "Invalid b3dm" << std::endl;
                    return NULL;
                }
        ...
        if (osg::getCpuByteOrder() == osg::BigEndian) {
            osg::swapBytes(header.version);
            osg::swapBytes(header.byteLength);
            osg::swapBytes(header.featureTableJSONByteLength);
            osg::swapBytes(header.featureTableBinaryByteLength);
            osg::swapBytes(header.batchTableJSONByteLength);
            osg::swapBytes(header.batchTableBinaryByteLength);
        }
        ...
        if (header.featureTableJSONByteLength > 0)
        {
            ...
            OSG_DEBUG << "Read featureTableJson " << featureTableJson << std::endl;
            ...
            OSG_DEBUG << "Read rtc_center " << rtc_center.x() << ", " << rtc_center.y() << ", " << rtc_center.z() << std::endl;
            */
        ...
        if (header.featureTableBinaryByteLength > 0)
        {
            ...
            OSG_DEBUG << "Read featureTableJson " << featureTableBinary << std::endl;
        ...
        if (header.batchTableJSONByteLength > 0)
        {
            ...
            OSG_DEBUG << "Read batchTableJSON " << batchTableJSON << std::endl;
        ...
        if (header.batchTableBinaryByteLength > 0)
        {
            ...
            OSG_DEBUG << "Read batchTableJSON " << batchTableBinary << std::endl;
        ...
        if (!err.empty())
            OSG_WARN << "GLTF ERROR: " << err << std::endl;
        if (!warn.empty())
            OSG_WARN << "GLTF WARNING: " << warn << std::endl;
